...
**适用场景**：技术文档类 wiki（API / schema / 规范说明等），英文占比高（≥ 35%）、术语密集。inline 双语会切碎阅读节奏，需要一份"纯中文版"在旁边。

**触发条件**（满足任一，自动建议产纯中文版）：

| 条件 | 阈值 | 说明 |
|---|---|---|
| 英文占比 ≥ 40% | 强建议 | 自然语言被英文术语切碎，纯中文版价值大 |
| 英文占比 35-40% | 弱建议 | 看具体类型，技术文档建议做 |
| 英文占比 < 35% | 不做 | inline 双语注就够，产纯中文版收益低 |
| frontmatter 显式 `cn_version_required: true` | 强制 | 你标记的必产中文版 |

**双轨结构**：

```
wiki/<name>.md          # 原版（single source of truth）
├── frontmatter: cn_version: ./<name>-cn.md
└── 双语 inline（英文 + 短译注）

wiki/<name>-cn.md       # 纯中文版（从原版生成）
├── frontmatter: generated_from / generated_at / translation_status: cn_only
└── 全文中文 + 保留代码标识符
```
...
|---|---|
| 人名 | 章节标题里的英文短语 |
| 公司名（Anthropic / OpenAI / Google） | 描述句 / 解释 / 注释 |
| 产品名 | 表格列名 |
| API / schema type 名 | 行话（case study → 案例 / gotcha → 坑） |
| JSON / API 字段名（`headline` 等） | 状态词（eligible → 够格 等） |
| 技术缩写（JSON-LD / SSR / CTR / API / URL / HTML） | TL;DR 表格内容 |
| 代码块内全部内容（**包括代码里的英文注释**） | 所有自然语言段落 |
| URL / 文件路径 | 列表项内的自然语言 |

**同步策略**：**原版为源 + 重生中文版**。原版改了 → 重新生成中文版（覆盖）。**不直接手改中文版**（改了会被下次重生覆盖）。

**铁律**：
- ✅ 文件名规则：`<原 wiki 名>-cn.md`（同目录，固定后缀）
- ✅ 原版 frontmatter 加 `cn_version: ./<原名>-cn.md` 反向指
- ✅ 中文版 frontmatter 加 `generated_from` + `generated_at` + `translation_status: cn_only`
- ✅ 章节结构一一对应（章节数 / 表格 / 代码块完全对齐，只翻自然语言）
- ❌ 不擅自调整章节顺序 / 增删表格（同步靠对齐）
- ❌ 不翻代码块 — JSON / JS / shell 等保留原文
- ❌ 不删原版的英文 — 原版仍是 inline 双语，纯中文版只是 derived view
- ✅ 老 wiki 批量做时，从英文占比最高开始，做完几份停下让你审过再继续（防止一口气做完发现风格不对要全返工）

### 翻译纯度按 wiki 类型分级

不同类型 wiki 的"保留原文"和"翻译"边界不一样。下表是一批英语技术内容实战出来的边界：

| Wiki 类型 | 翻译重点 | 保留原文 |
|---|---|---|
| **概念 / 技术文档** | 描述句 / 解释 / 章节标题 / 行话 | schema / API type 名 / 字段名 / 代码块 / 工具名 / 缩写 |
| **博客索引** | 章节标题 / 时代描述 / 反查指南 | **博客文章标题作为引用标识符，保留原文 + 括号注**（翻译标题会失去溯源能力） |
| **人物目录** | 角色描述 / 团队描述 / 分类标题 | 人名 / 公司名 / 职位英文术语（首次出现给中文释义） |
| **访谈 / Q&A** | 描述 / 框架 / 表格 / 元数据 | **引言原句保留 + 下方加 `*译：中文*`**（精度可追溯） |

**判断**：wiki 是否以**引用形态的英文**（文章标题 / 引言 / 公开命名实体）为主？是 → 保留比例高；否（概念解释为主）→ 翻译比例高。

### 批量做纯中文版的工作流

**触发**：你说"批量做英语源 wiki 纯中文版" 或"挑英文占比高的 wiki 做中文版"。
...
1. **扫一遍英文占比**（Bash 一行命令）：
   ...
   for f in ~/kb/wiki/*.md; do
     [[ "$f" == *-cn.md ]] && continue
     total=$(wc -c < "$f")
     ascii=$(LC_ALL=C grep -o '[[:alpha:]]' "$f" | wc -l)
     echo "$(awk "BEGIN{printf \"%.1f\", $ascii*100/$total}")% $f"
   done | sort -rn | head -30
   ```

2. **按英文占比降序排清单**，分批（每批 3-5 份）
3. **第一批先做最高占比的几份**（高 ROI：最痛 + 验证翻译纯度边界）
4. **commit + push + 报战绩**（每份英文占比变化数字）
5. **暂停让你审过**再进下一批（校准规则后，后续批一致性更好）
6. **每批 commit message** 列每份 X% → Y% 变化（git log 可读）

**铁律**：不一口气做完所有候选 — 风格漂移风险大，审过再继续。
